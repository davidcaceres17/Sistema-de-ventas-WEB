...
public class sr_usuario extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            loguear = new login(request.getParameter("usuario"),request.getParameter("contra"));
           
            // Boton agregar 
            if ("verificar".equals(request.getParameter("btn_verificar"))){
             if (loguear.verificar()>0){
             response.sendRedirect("./admin/");
             
             }else{
             response.sendRedirect("./user/?entro=0");
             }
             }
            /*
            // Boton modificar 
            if ("modificar".equals(request.getParameter("btn_modificar"))){
             ...
             response.sendRedirect("./Mantenimientos/menu.jsp");
             
             }else{
             out.println("<h1> xxxxxxx No se Modifico xxxxxxxxxxxx </h1>");
             out.println("<a href='./Mantenimientos/menu.jsp'>Regresar...</a>");
             }
             }
